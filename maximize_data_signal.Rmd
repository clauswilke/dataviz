...
pad_figure(stamp_ugly(price_plot + theme_gray(12)))
```
...
pad_figure(stamp_bad(price_plot + theme_half_open()))
```
...
pad_figure(price_plot + theme_minimal_hgrid())
```
...
```{r Aus-athletes-grid, fig.height = 7 * 0.618, fig.cap='(ref:Aus-athletes-grid)'}
...
pad_figure(plot_grid(nrg_plot, NULL, nrow=1, rel_widths=c(1, 0.2)))
```
...
pad_figure(stamp_bad(plot_grid(nrg_plot2, NULL, nrow=1, rel_widths=c(1, 0.2))))
```
