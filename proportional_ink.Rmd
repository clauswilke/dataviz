...
pad_figure(stamp_bad(p_income_bad))
```
...
pad_figure(stamp_bad(fb_drop_bad))
```
...
p <- ggplot(hawaii_income_diff, aes(x = reorder(county, desc(filter(hawaii_income, year == 2015)$median_income)),
                               y = income_diff)) + 
  geom_col(fill = "#56B4E9") +
  xlab("county") +
...
```{r oceania-gdp-logbars, fig.height = 7 * 0.5, fig.cap = '(ref:oceania-gdp-logbars)'}
library(gapminder)
...
pad_figure(stamp_bad(oc_bad), fig.width = 7)
```
...
```{r oceania-gdp-logbars-long, fig.height = 7 * 0.5, fig.cap = '(ref:oceania-gdp-logbars-long)'}
...
pad_figure(stamp_bad(oc_bad2), fig.width = 7)
```
...
```{r oceania-gdp-dots, fig.height = 7 * 0.5, fig.cap = '(ref:oceania-gdp-dots)'}
p <- ggplot(df_oceania, aes(x = reorder(country, -GDP), y = log10(GDP))) + 
...
```{r oceania-gdp-relative, fig.height = 7 * 0.5, fig.cap = '(ref:oceania-gdp-relative)'}
...
p <- ggplot(df_oceania_ratios, aes(x = reorder(country, gdp_ratio), y = gdp_ratio)) + 
  geom_col(fill = "#56B4E9") + 
...
```{r RI-pop-bars, fig.height = 5.5 * 0.618, fig.cap = '(ref:RI-pop-bars)'}
p <- ggplot(RI_pop, aes(x = factor(county, levels = rev(county)), y = pop2010, fill = county)) + 
  geom_col() +
...
```{r RI-pop-treemap, fig.height = 4.5 * 0.75, fig.cap = '(ref:RI-pop-treemap)'}
