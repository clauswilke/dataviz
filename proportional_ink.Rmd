...
Similar visualization problems frequently arise in the visualization of time series, such as stock price. Figure \@ref(fig:fb-stock-drop-bad) suggests a massive collapse in the stock price of Facebook occurred around Nov. 1, 2016. In reality, the price decline was moderate relative to the total price of the stock (Figure \@ref(fig:fb-stock-drop-good)).
...
In the two preceding examples, the data values were encoded by the bar height or area height, measured as the distance from the *y* value of the data to *y* = 0. **some more text here.**
...
Whenever we are visualizing data along a linear scale, as we did in the previous subsection, the areas of bars, rectangles, or other shapes are automatically proportional to the data values. The same is not true if we're using a logarithmic scale. 
...
All preceding examples visualized data along one linear dimension, so that the data value was encoded both by area and by location along the *x* or *y* axis. Other visualizations represent the data value only by area. The most common one is the pie chart (Figure \@ref(fig:RI-pop-pie)). Even though technically the data values are mapped onto angles, which are represented by location along a circular axis, in practice we are often not judging the angles of a pie chart. Instead, the first visual property we notice is the size of the areas of the pie wedges.

(ref:RI-pop-pie) Number of inhabitants in Rhode Island counties, shown as a pie chart. Data source: 2010 Decennial U.S. Census.
...
Because the area of each pie wedge is proportional to its angle which in turn is proportional to the data value the wedge represents, pie charts satisfy the principle of proportional ink. However, we perceive the area in a pie chart differently from the same area in a bar plot. 

(ref:RI-pop-bars) Number of inhabitants in Rhode Island counties, shown as bars. Data source: 2010 Decennial U.S. Census.

```{r PI-pop-bars, fig.width = 5.5, fig.cap = '(ref:RI-pop-bars)'}
ggplot(RI_pop, aes(x = factor(county, levels = rev(county)), y = pop2010, fill = county)) + 
  geom_col() +
  scale_fill_OkabeIto(darken = -.4, guide = "none") +
  scale_y_continuous(expand = c(0, 0), labels = label_log10,
...
(ref:RI-pop-treemap) Number of inhabitants in Rhode Island counties, shown as a treemap. Data source: 2010 Decennial U.S. Census.
