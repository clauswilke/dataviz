...
```{r}
df_rhode_island <- filter(US_census, state == "Rhode Island") %>%
  select(county = name,
         pop2000, pop2010) %>%
  gather(popyear, population, -county) %>%
  extract(popyear, "year", regex = "([[:digit:]]+)")

ggplot(filter(df_rhode_island, year == 2010), aes(x = reorder(county, desc(population)), y = population)) +
  geom_col() +
  scale_y_continuous(labels = label_log10, expand = c(0, 0)) +
  xlab("county") +
  theme_minimal_hgrid()
```
...
ggplot(df_hawaii, aes(x = county, y = population, fill = year)) +
