...
```{r}
ggplot(filter(hawaii_income, year == 2015), aes(x = reorder(county, desc(median_income)), y = median_income)) +
  geom_col() +
...
ggplot(filter(hawaii_income, year == 2015), aes(x = reorder(county, desc(median_income)), y = median_income)) +
  geom_col() +
  coord_cartesian(xlim = c(0.5, 5.55), ylim = c(0, 78000), expand = FALSE) +
  xlab("county") +
  scale_y_continuous(name = "median income", 
                     breaks = 20000*(0:3),
                     labels = function(x) paste0("$", scales::comma(x))) +
...
ggplot(hawaii_income_diff, aes(x = reorder(county, desc(filter(hawaii_income, year == 2015)$median_income)),
  ...
  geom_col() +
  xlab("county") +
  scale_y_continuous(name = "change in median income", 
                     limits = c(-5000, 25000),
                     expand = c(0, 0),
                     labels = function(x) paste0("$", scales::comma(x))) +
  theme_minimal_hgrid()
...
```{r}
df_hawaii <- filter(US_census, state == "Hawaii") %>%
  select(county = name,
         pop2000, pop2010) %>%
  gather(popyear, population, -county) %>%
  extract(popyear, "year", regex = "([[:digit:]]+)") %>%
  extract(county, "county", regex = "(.+) County")

ggplot(filter(df_hawaii, year == 2000), aes(x = county, y = population)) +
  geom_col() +
  theme_minimal_hgrid() +
  scale_y_sqrt()
