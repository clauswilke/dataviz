...
```{r boxoffice-vertical, fig.height = 8 * 0.5, fig.cap = '(ref:boxoffice-vertical)'}
p <- ggplot(boxoffice, aes(x = fct_reorder(title_short, rank), y = amount)) +
...
```{r boxoffice-rot-axis-tick-labels, fig.height = 6*0.85, fig.cap = '(ref:boxoffice-rot-axis-tick-labels)'}
boxoffice %>%
...
pad_figure(stamp_ugly(p_box_axrot), fig.width = 6)
```
...
p <- ggplot(boxoffice, aes(x = fct_reorder(title_short, desc(rank)), y = amount)) +
  geom_col(fill = "#56B4E9", alpha = 0.9) +
...
pad_figure(stamp_bad(p), fig.width = 6)
```
...
pad_figure(stamp_bad(p_income_sorted), fig.width = 6)
```
...
```{r income-by-age-race-dodged, fig.height = 8 * 0.5, fig.cap = '(ref:income-by-age-race-dodged)'}
...
pad_figure(p_income_race_dodged, fig.width = 8)
```
...
```{r income-by-race-age-dodged, fig.height = 8 * 0.4, fig.cap = '(ref:income-by-race-age-dodged) '}
...
pad_figure(p_income_age_dodged, fig.width = 8)
```
...
```{r income-by-age-race-faceted, fig.height = 8.5 * 0.618, fig.cap = '(ref:income-by-age-race-faceted)'}
income_df %>%
...
pad_figure(p_income_age_dodged, fig.width = 8.5)
```
...
```{r titanic-passengers-by-class-sex, fig.height = 5.5 * 0.618, fig.cap = '(ref:titanic-passengers-by-class-sex)'}
...
p <- ggplot(titanic_groups, aes(x = class, y = n, fill = sex)) +
...
```{r Americas-life-expect, fig.height = 7 * 0.8, fig.cap = '(ref:Americas-life-expect)'}
library(gapminder)
...
```{r Americas-life-expect-bars, fig.height = 7 * 0.8, fig.cap = '(ref:Americas-life-expect-bars)'}
...
pad_figure(stamp_bad(life_bars), fig.width = 7)
```
...
```{r Americas-life-expect-bad, fig.height = 7 * 0.8, fig.cap = '(ref:Americas-life-expect-bad)'}
...
pad_figure(stamp_bad(p), fig.width = 7)
```
...
```{r internet-over-time, fig.height = 8.5 * 0.618, fig.cap = '(ref:internet-over-time)'}
...
```{r internet-over-time2, fig.height = 8.5 * 0.618, fig.cap = '(ref:internet-over-time2)'}
internet_summary <- internet_short %>%
  group_by(country) %>%
