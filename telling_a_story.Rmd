...
```{r arrival-delay-vs-distance, fig.cap = '(ref:arrival-delay-vs-distance)'}
...
#pad_figure(stamp_bad(p_delay_distance), fig.width = 8) 
```
...
flights_grouped %>%
  ...
  ggplot(aes(x=reorder(name, desc(mean_delay)), y=mean_delay, fill = highlight)) + 
    ...
    coord_flip() +
    theme_minimal_vgrid() +
    theme(axis.ticks.y = element_blank()) -> p
...
```{r united-departures-weekdays, fig.width = 4, fig.asp = 1, fig.cap = '(ref:united-departures-weekdays)'}
...
```{r all-departures-weekdays, fig.width = 5, fig.asp = 0.4, fig.cap = '(ref:all-departures-weekdays)'}
ggplot(flights_grouped, aes(x = wday(time_hour, label = TRUE, week_start = 1))) + 
  geom_bar(fill = "#0072B2D0", color = "white", size = 0.3) + 
  facet_grid(origin ~ name) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 7500)) +
  scale_x_discrete(labels = c("M", "", "W", "", "F", "", "S"),
                   expand = c(0, 0.05),
                   name = "weekday") +
  theme_minimal_hgrid(7) +
  #panel_border() +
...
```{r athletes-composite-inconsistent, fig.cap = '(ref:athletes-composite-inconsistent)'}
...
p1 <- ggplot(athletes_df, aes(x = sex)) + 
  ...
  theme(axis.title = element_text(size = 0.857*12),
        axis.line.x = element_blank(),
...
p2 <- ggplot(athletes_df, aes(x = rcc, y = wcc, shape = sex, color = sex, fill = sex)) + 
  ...
  scale_y_continuous(limits = c(2.2, 11.5), expand = c(0, 0)) +
...
```{r athletes-composite-good, fig.cap = '(ref:athletes-composite-good)'}
...
```{r athletes-composite-repetitive, fig.cap = '(ref:athletes-composite-repetitive)'}
cc_df <- athletes_df %>%
  select(sex, rcc, wcc) %>%
