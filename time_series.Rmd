...
*Somewhere state that we have already seen connected scatter plot in Chapter \@ref(coordinate-systems-axes).*
...
In a connected scatter plot, it is important that we indicate the direction (done via gradually darkening color in Figure \@ref(fig:house-price-path)) and temporal scale (done via labeling of specific time points in Figure \@ref(fig:house-price-path)). Without these hints, we can make out neither the direction nor the speed of change of the time series shown (Figure \@ref(fig:house-price-path-bad)).
...
**Need a connecting sentence here.**
Viewers are more likely to confuse order and direction in a connected scatter plot than in line graphs and less likely to report correlation. However, connected scatter plots seem to result in higher viewer engagement, and thus such plots may be a effective tools to draw readers into a story [@Haroz_et_al_2016].

Even though connected scatter plots can show only two variables at a time, we can use them to visualize higher-dimensional datasets. The trick is to apply dimension reduction first, as discussed in Chapter \@ref(visualizing-associations). We can then draw a connected scatterplot in the dimension-reduced space. As an example of this approach, we will visualize a database of monthly observations of over 100 macroeconomic indicators, provided by the Federal Reserve Bank of St. Louis. We perform a principal components analysis (PCA) of all indicators and then draw a connected scatter plot of principal component (PC) 2 versus PC 1 (Figure \@ref(fig:fred-md-PCA)a) and versus PC 3 (Figure \@ref(fig:fred-md-PCA)b) for data from January 1990 to December 2017.

(ref:fred-md-PCA) Visualizing a high-dimensional time series as a connected scatter plot in principal components space. **more figure caption** Data source: M. W. McCracken, St. Louis Fed
