...
ggplot(filter(house_prices, state == "California", year(date) > 2000),
        aes(unemploy_perc, house_price_perc, alpha = date)) + 
  geom_path() +
  scale_x_continuous(name = "Unemployment rate (percent)") +
  scale_y_continuous(name = "12-month percent change in house prices") + 
  guides(alpha = FALSE) +
  theme_dviz_grid()
```
...
```{r}
MASS::oats %>% 
  mutate(N = 100*as.numeric(sub("cwt", "", N, fixed = TRUE))) %>%
  group_by(N, V) %>%
...
ggplot(oats_df,
  ...
  scale_x_continuous(name = "manure treatment (lbs/acre)") +
  scale_shape_manual(values = c(21, 22, 23),
                     name = "variety") + 
  scale_color_manual(values = c("#0072b2", "#D55E00", "#009e73"),
                     name = "variety") +
