...
```{r corruption-development, fig.height = 7 * 0.7, fig.cap = '(ref:corruption-development)'}
...
corruption %>% filter(year == 2015) %>% na.omit() %>%
  ...
  ggplot(aes(cpi, hdi)) + 
    ...
    theme(legend.position = "top",
          ...
          plot.margin = margin(0, 7, 0, 7)) -> plot_corrupt_base

cur_dev <- grDevices::dev.cur()
grDevices::pdf(NULL, width = 7, height = 4.9)
grob_corrupt_base <- grid::forceGrob(ggplotGrob(plot_corrupt_base))
null <- grDevices::dev.off()
null <- grDevices::dev.set(cur_dev)

pad_figure(grob_corrupt_base, fig.width = 7)
```
...
```{r corruption-development-infographic, fig.height = 7 * 0.84, fig.cap = '(ref:corruption-development-infographic)'}
...
plot_corrupt_caption <- ggplot() +
  ...
  theme(plot.caption = element_text(size = 10, hjust = 1),
        plot.margin = margin(0, 7, 7, 7))

pad_figure(plot_grid(plot_corrupt_title, grob_corrupt_base, plot_corrupt_caption,
                     ncol = 1, rel_heights = c(.15, 1, .05)),
           fig.width = 7)
```
