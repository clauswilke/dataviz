...
pad_figure(stamp_bad(lincoln_errbar))
```
...
pad_figure(plot_grid(p_points, p_boxplot, rel_widths = c(.65, 1), nrow = 1))
```
...
pad_figure(plot_grid(p_points, p_violin, rel_widths = c(.65, 1), nrow = 1))
```
...
pad_figure(stamp_bad(lincoln_points))
```
...
p <- ggplot(lincoln_df, aes(x = `Mean Temperature [F]`, y = `Month`)) +
...
p_binline <- ggplot(lincoln_df, aes(x = `Mean Temperature [F]`, y = `Month`)) +
  stat_binline(scale = 3, bins = 17, draw_baseline = FALSE,
...
pad_figure(stamp_ugly(p_binline)) 
```
...
```{r movies-ridgeline, fig.height = 5 * 1, fig.cap = '(ref:movies-ridgeline)'}
p <- ggplot(movie_lengths, aes(x = length, y = year, group = year)) +
...
```{r dw-nominate-ridgeline, fig.height = 8.5 * 0.5, fig.cap = '(ref:dw-nominate-ridgeline)'}
# U.S. House 1963-2013
