...
p <- ggplot(bund_pie) + 
  geom_arc_bar(aes(x0 = 0, y0 = 0, r0 = 0, r = rpie,
                   start = start_angle, end = end_angle, fill = party), color = "white", size = 0.5) +
  geom_text(aes(x = rlabel*sin(mid_angle), y = rlabel*cos(mid_angle), label = party,
                hjust = hjust, vjust = vjust)) +
  geom_text(aes(x = 0.6*sin(mid_angle), y = 0.6*cos(mid_angle), label = seats),
            color = c("black", "white", "white")) +
  coord_fixed() +
  scale_x_continuous(limits = c(-1.5, 1.5), expand = c(0, 0), name = "", breaks = NULL, labels = NULL) +
  scale_y_continuous(limits = c(-1.01, 1.15), expand = c(0, 0), name = "", breaks = NULL, labels = NULL) +
...
```{r, bundestag-stacked-bars, fig.height = 8.5 * 0.618, fig.cap='(ref:bundestag-stacked-bars)'}
...
bt_bars_stacked <- insert_yaxis_grob(bt_bars_stacked_base + theme_minimal_hgrid(),
...
```{r marketshare-pies, fig.height = 8.5 * 0.35, fig.cap='(ref:marketshare-pies)'}
# calculate the start and end angles for each pie
...
pad_figure(stamp_bad(stacked_bars))
```
...
p <- ggplot(marketshare, aes(x = company, y = percent, fill = company)) + 
  geom_col() + 
  facet_wrap(~year) +
  scale_y_continuous(expand = c(0, 0)) +
...
```{r women-parliament, fig.height = 7.5 * 0.55, fig.cap = '(ref:women-parliament)'}
ccode = "RWA" # Rwanda
#ccode = "BEL" # Belgium
...
plot_base <- ggplot(women_rwanda, aes(x = year, y = percent, fill = gender)) +
    ...
    #theme_minimal_hgrid() + background_grid(major = "none", minor = "none")
...
pad_figure(insert_yaxis_grob(plot_base, yax, grid::unit(.1, "null")),
           fig.width = 7.5)
...
```{r health-vs-age, fig.height = 6 * 0.5, fig.cap='(ref:health-vs-age)'}
...
df_marital <- select(productplots::happy, age, marital) %>%
  na.omit() %>%
...
pad_figure(p_aligned[[1]], fig.width = 6)
```
...
```{r health-vs-age-facets, fig.height = 8.5 * 0.35, fig.cap='(ref:health-vs-age-facets)'}
...
```{r marital-vs-age, fig.height = 6 * 0.5, fig.cap='(ref:marital-vs-age)'}
pad_figure(stamp_bad(p_aligned[[2]]), fig.width = 6)
```
...
```{r marital-vs-age-facets, fig.height = 8.5 * 0.35, fig.cap='(ref:marital-vs-age-facets)'}
