...
```{r popgrowth-vs-popsize-colored, fig.height = 7 * 2 * 0.618, fig.cap = '(ref:popgrowth-vs-popsize-colored)'}
...
pad_figure(stamp_bad(plot_grid(NULL, p_comb, NULL, rel_widths = c(.5, 6, .5), nrow = 1)),
           fig.width = 7)
```
...
```{r popgrowth-vs-popsize-bw, fig.height = 8.5 * 0.618, fig.cap = '(ref:popgrowth-vs-popsize-bw)'}
library(ggrepel)
...
ggplot(popgrowth_df, aes(x = pop2000, y = 100*popgrowth, color = region)) +
  ...
  geom_text_repel(data = df_repel,
                  ...
                  segment.alpha = 0.5, point.padding = 0.45,
                  box.padding = .4,
                  force = 1,
...
```{r popgrowth-US-rainbow, fig.height = 6.5 * 1.2, fig.cap = '(ref:popgrowth-US-rainbow)'}
...
pad_figure(stamp_bad(popgrowth_bars_rainbow), fig.width = 6.5)
```
...
```{r rainbow-desaturated, fig.height = 6.5 * 2 * 0.14, fig.cap = '(ref:rainbow-desaturated)'}
p1 <- gg_color_gradient() + scale_fill_gradientn(colors = rainbow(10)) + ggtitle("rainbow scale")
p2 <- gg_color_gradient() + scale_fill_gradientn(colors = desaturate(rainbow(10))) + ggtitle("rainbow converted to grayscale")

pad_figure(plot_grid(p1, p2, ncol = 1), fig.width = 6.5)
```
...
```{r map-Texas-rainbow, fig.height = 7 * 0.75, fig.cap = '(ref:map-Texas-rainbow)'}
library(sf)
...
pad_figure(stamp_bad(texas_rainbow), fig.width = 7)
```
...
```{r heat-cvd-sim, fig.height = 6.5 * 2 * 0.14, fig.cap = '(ref:heat-cvd-sim)'}
grad_heat <- gg_color_gradient(plot_margin = margin(12, 0, 0, 0),
                                    ymargin = 0.05) + 
               scale_fill_continuous_sequential("Heat")

pad_figure(cvd_sim(grad_heat), fig.width = 6.5)
```
...
```{r red-green-cvd-sim, fig.height = 6.5 * 2 * 0.14, fig.cap = '(ref:red-green-cvd-sim)'}
...
pad_figure(cvd_sim(grad_red_green), fig.width = 6.5)
```

(ref:blue-green-cvd-sim) A blue--green contrast becomes indistinguishable under blue--yellow cvd (tritanomaly).

```{r blue-green-cvd-sim, fig.height = 6.5 * 2 * 0.14, fig.cap = '(ref:blue-green-cvd-sim)'}
...
```{r PiYG-cvd-sim, fig.height = 6.5 * 2 * 0.14, fig.cap = '(ref:PiYG-cvd-sim)'}
grad_red_green <- gg_color_swatches(n = 5, plot_margin = margin(12, 0, 0, 0),
                                    ymargin = 0.05) + 
...
```{r palette-Okabe-Ito, fig.height = 8.5 * 0.14, fig.cap = '(ref:palette-Okabe-Ito)'}
...
```{r colors-thin-lines, fig.height = 6.5 * 0.8, fig.cap = '(ref:colors-thin-lines)'}
...
pad_figure(cvd_sim(p, label_x = 0.0725, label_y = 0.98), fig.width = 6.5)
```
