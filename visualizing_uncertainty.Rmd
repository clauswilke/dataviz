...
We also need to consider the audience: For a lay audience, using a visualization strategy that creates a strong intuitive impression of the uncertainty will be most important, even if this comes at the cost of either visualization accuracy or total amount of data conveyed. By contrast, for an audience of experts, e.g. in a scientific publication, accuracy and data density may be more important.

## Framing probabilities as frequencies

When we use the word "uncertainty," we imply that we don't exactly know something we would like to know. Uncertainty is most easily understood in the context of future events. If I am going to flip a coin I can't know ahead of time what the outcome will be. The eventual outcome is uncertain. Uncertainty can also exist about events in the past, however. If yesterday I looked out of my kitchen window exactly twice, once at 8am and once at 4pm, and I saw a red car parked across the street at 8am but not at 4pm, then I can conclude the car left at some point during the eight-hour window but I don't know exactly when. It could have been 8:01am, 9:30am, 2pm, or any other time during those eight hours.
...
```{r probability-waffle, fig.asp = 1.2/3, fig.cap = '(ref:probability-waffle)'}
...
ggplot(df_norm, aes(x, y)) +
  ...
  scale_x_continuous(
    name = "percentag point advantage for blue",
...
```{r confidence-visualizations, fig.width = 8.5, fig.asp = 0.75, fig.cap = '(ref:confidence-visualizations)'}
...
```{r bayes-ridgeline, fig.asp = 0.6, fig.cap = '(ref:bayes-ridgeline)'}
bayes_emmeans %>%
...
(ref:chocolate-HOP-animated) (for online edition) Hypothetical outcome plot for chocolate bar ratings of Canadian and U.S. manufactured bars. Each vertical green bar represents the rating for one bar, and each frame of the animation shows a comparison of two randomly chosen bars, one Canadian and one U.S.
...
As a second example, consider the variation in shapes among equally probable trendlines in Figure \@ref(fig:mpg-uncertain)b. Because all trendlines are plotted on top of one another, in Figure \@ref(fig:mpg-uncertain)b we primarily perceive the overall area that is covered by trendlines, similar to the confidence band. Perceiving individual trendlines is difficult. By turning this figure into a HOP, we can highlight individual trendlines one at a time, which helps with perceiving their individual shapes (Figure \@ref(fig:mpg-uncertain-HOP-static)/Figure \@ref(fig:mpg-uncertain-HOP-animated)).
...
```{r mpg-uncertain-HOP-static, fig.width = 8.5, fig.asp = 2/4, fig.cap = '(ref:mpg-uncertain-HOP-static)'}
#set.seed(8692282)
set.seed(8692276)
...
(ref:mpg-uncertain-HOP-animated) (for online edition) Hypothetical outcome plot for fuel efficiency versus displacement. Each dot represents one car, and the smooth lines were obtained by fitting a cubic regression spline with 5 knots. Each line in each frame of the animation represents one alternative fit outcome, drawn from the posterior distribution of the fit parameters.
...
When switching from one frame to another in the HOP animation, there are two ways in which we can proceed: First, we can execute a hard switch, as in a slide projector. We show one outcome for some time period, say 0.5 seconds, and then the next one, and so on. Second, we can smoothly interpolate between frames, e.g., slowly deform the trendline from one frame until it looks like the trendline from another frame. In my personal opinion, the smooth transition is preferable, because it again reduces the risk of deterministic construal error. By seeing a wide range of intermediate states, even if some of them are not proper hypothetical outcomes, we are reminded that there is a continuum of different outcomes that all could occur.

There is one critical aspect we need to pay attention to when preparing a HOP: We need to make sure that the outcomes we do show are representative of the true distribution of possible outcomes. Otherwise, our HOP could be rather misleading. For example, going back to the case of chocolate ratings, if I randomly selected ten outcome pairs of chocolate bars and among those the U.S. bar was rated higher than the Canadian bar in seven cases, then the HOP would erroneously create the impression that U.S. bars tend to win against Canadian bars in a direct head-to-head competition. We can prevent this issue either by choosing a very large number of outcomes, so sampling biases are unlikely, or by verifying in some form that the outcomes that are shown are appropriate. For example, in Figure \@ref(fig:chocolate-HOP-static)/Figure \@ref(fig:chocolate-HOP-animated), I verified that the number of times the Canadian bar was shown winning was close to the true percentage of 53%.
