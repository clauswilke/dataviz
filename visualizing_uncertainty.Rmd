...
*Intro sentences, uncertainty is related to probability.*

Probability is most easily understood as a frequency in the context of many repeated trials. If a particular outcome occurs with a probability of 10%, then we expect that among many repeated trials that outcome will be seen in one out of ten cases.
...
In Figure \@ref(fig:cocoa-CI-vs-n), I am showing error bars representing three different confidence intervals at the same time, using darker colors and thicker lines for intervals representing lower confidence levels. *discuss: humans tend to take whatever they see at face value; this heightens the sense of uncertainty. determinstic construal error.*
...
The chocolate bars from Switzerland have a higher mean rating than those from the U.S.A., but how confident are we that this difference reflects a real difference in chocolate flavor and not just random variation associated with sampling a finite number of bar? Judging this from Figure \@ref(fig:mean-chocolate-ratings) is difficult, because both the mean Swiss rating and the mean U.S. rating have uncertainty, and both uncertainties matter to the question whether the means are different. Some people attempt to use rules of thumb to judge significance in difference from the extent to which error bars do or don't overlap, but this should be a practice of last resort.

The correct way to address whether there are differences in mean rating is to calculate confidence intervals for the differences. If those confidence intervals exclude zero, then we know the difference is significant at the respective confidence level. For the chocolate ratings dataset, we see that only bars from Canada are significantly higher rated than bars from the U.S. (Figure \@ref(fig:chocolate-ratings-contrasts)). For bars from Switzerland, the 95% confidence interval on the difference just barely includes the value zero. Thus, there is a slightly bigger than 5% chance that the observed difference between mean ratings of U.S. and Swiss chocolate bars represents nothing more than sampling variation.
...
Error bars can be added to a variety of different types of plots to indicate uncertainty. For example, we can show amounts with uncertainty by drawing a bar plot with error bars (Figure \@ref(fig:butterfat-bars)). *one more sentence; not necessarily the best choice*. We can also draw error bars along both the *x* and the *y* direction in a scatter plot (Figure \@ref(fig:median-age-income)). 


(ref:butterfat-bars) Mean butterfat contents in the milk of four cattle breads. Error bars indicate +/- one standard error of the mean. Visualizations of this type are frequently seen in the scientific literature. While they are technically correct, they represent neither the variation within each sample nor the uncertainty of the sample mean particularly well. See Figure \@ref(fig:butterfat-densitites) for the variation in butterfat contents within each sample. Data Source: Canadian Record of Performance for Purebred Dairy Cattle
...
*Discuss Bayesian inference briefly; mention that methods from visualizing distributions chapters apply.*

*Do a ridgeline example with shaded areas?*

```{r bayes-vs-ols, fig.asp = 0.6}
linear_results <-
...
#    expand = c(0, 0),
  ...
  scale_color_manual(
    ...
    breaks = c("OLS", "Bayes"),
    labels = c("ordinary least squares", "Bayesian estimation")
...
*Introduce concepts such as parameter, estimate, Bayesian analysis, frequentist analysis. Others?*

A Bayesian credible interval tells you about where the true parameter likely is, whereas a frequentist confidence interval tells you about where the true parameter likely not is. Upon first reading, this distinction may seem like semantics, but there are important differences between these two approaches. Under the Bayesian approach, you use the data and your prior knowledge about the system under study (called the *prior*) to calculate a probability distribution (called the *posterior*) that tells you where you can expect the true parameter value to lie. By contrast, under the frequentist approach, you first make an assumption that you intend to disprove. This assumption is called the *null hypothesis*, and it is usually simply the assumption that the parameter equals zero. You then calculate the probability that random sampling would generate data similar to what was observed if the null hypothesis were true. The confidence interval is a representation of this probability. If the confidence interval at, say 95% confidence, excludes the parameter value under the null hypothesis (i.e., the value zero), then you are at least 95% confident that the null hypothesis is false. Thus, to summarize, a Bayesian credible interval makes a statement about the true parameter value and a frequentist confidence interval makes a statement about the null hypothesis.
...
The discussion of whether Bayesian or frequentist approaches are preferable has endured for over a century, and it is unlikely to go away any time soon. For either approach, there are applications and settings where it is preferable. In Table \@ref(tab:estimation-frameworks), I provide a very rough overview of their respective strengths and weaknesses. From a purely practical perspective, Bayesian approaches often incur an additional level of modeling complexity and computational cost that may not be warranted or cost effective in certain applications. 

Table: (\#tab:estimation-frameworks) Comparisons of different approaches to parameter and uncertainty estimation. This table is meant as a rough guideline differentiating the most commonly used methods and procedures. Any particular analysis approach may deviate from the classification provided here.
...
bootstrap         moderate            moderate             low                minimal
------------------------------------------------------------------------------------------

In Table \@ref(tab:estimation-frameworks), I have also added the bootstrap. I would like to emphasize that while Bayesian and frequentist approaches are broad conceptual frameworks, the bootstrap is primarily a procedure, and it can be performed within either framework. In the classical, non-Bayesian setup, the bootstrap consists of resampling the data many times with replacement and performing the analysis of interest separately on each bootstrap sample. The variability in analysis outcomes among samples provides an estimate of the uncertainty inherent in the sample. The bootstrap is popular because it can be performed easily on any dataset and for any analysis. It is particularly useful in settings with novel, complex, or otherwise poorly understood statistics. Where Bayesian and frequentist approaches often require careful mathematical modeling and theory development, the boostrap only requires a powerful computer.
...
Hypothetical outcomes plots [@Hullman_et_al_2015] 

The true value is approximately 0.53.

We need to make sure to sample sufficiently many states that the presented sequence is actually representative. If we sampled only, say, 10 states it would be easy to create a non-representative sample that produces misleading results.

(ref:chocolate-HOP-static) (for print edition) *Figure caption*
...
(ref:chocolate-HOP-animated) (for online edition) *Figure caption*
...
(ref:mpg-uncertain-HOP-static) (for print edition) *Figure caption*
...
(ref:mpg-uncertain-HOP-animated) (for online edition) *Figure caption*
