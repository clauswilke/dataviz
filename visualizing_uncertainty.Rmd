...
All statisticians use samples to calculate parameter estimates and their uncertainties. However, statisticians are deeply divided in how they approach these calculations, into Bayesians and frequentists. Bayesians assume that they have some prior knowledge about the world, and they use the sample to update this knowledge. By contrast, frequentists attempt to make precise statements about the world without any prior knowledge in hand. Fortunately, in terms of visualizing uncertainty, most approaches apply---with minor modifications---in both Bayesian and frequentist settings. Here, I will first discuss the frequentist approach and then describe what we may want to do differently in a Bayesian context.

Frequentists most commonly visualize uncertainty with error bars. While error bars can be useful as a visualization of uncertainty, they are not without problems, as I already alluded to in Chapter \@ref(boxplots-violins) (see Figure \@ref(fig:lincoln-temp-points-errorbars)). It is easy for readers to be confused about what an error bar represents. To highlight this problem, in Figure \@ref(fig:cocoa-data-vs-CI) I show five different uses of error bars for the same dataset. The dataset contains expert ratings of chocolate bars, rated on a scale from 1 to 5, for chocolate bars manufactured in a number of different countries. For Figure \@ref(fig:cocoa-data-vs-CI) I have selected specifically the ratings for chocolate bars manufactured in Canada. Underneath the sample, which is shown as a strip chart, we see the mean plus/minus the standard deviation of the sample, the mean plus/minus the standard error, and 80%, 95%, and 99% confidence intervals. *one more sentence here?*
...
The standard error is approximately given by the sample standard deviation divided by the square root of the sample size. (This relationship follows from the central limit theorem.) Therefore, larger samples *continue here. Figure \@ref(fig:cocoa-CI-vs-n).*
...
Error bars are convenient because they allow us to show many estimates and their uncertainties all at once. *Continue here. Figure \@ref(fig:mean-chocolate-ratings)*
...
Error bars are in general not a suitable means to determine whether two groups are different. *Continue here. Figure \@ref(fig:chocolate-ratings-contrasts)*
...
ggplot(cow_means, aes(x = breed, y = mean, ymin = mean - 1.96*se, ymax = mean + 1.96*se)) +
  ...
  geom_errorbar(size = 0.75, width = 0.1) +
  scale_x_discrete(
...
(ref:blue-jays-male-conf-band)  *Figure caption*
...
(ref:blue-jays-male-fitted-draws) *Figure caption*
...
(ref:blue-jays-male-graded-conf-band) *Figure caption*
