...
```{r mpg-cty-displ-solid, fig.height = 5.5 * 0.7416, fig.cap='(ref:mpg-cty-displ-solid)'}
...
pad_figure(stamp_bad(p_mpg_solid), fig.width = 5.5)
```
...
```{r mpg-cty-displ-transp, fig.height = 5.5 * 0.7416, fig.cap='(ref:mpg-cty-displ-transp)'}
...
pad_figure(p_mpg_transp, fig.width = 5.5)
```
...
```{r mpg-cty-displ-jitter, fig.height = 5.5 * 0.7416, fig.cap='(ref:mpg-cty-displ-jitter)'}
...
pad_figure(p_mpg_jitter, fig.width = 5.5)
```
...
```{r mpg-cty-displ-jitter-extreme, fig.height = 5.5 * 0.7416, fig.cap='(ref:mpg-cty-displ-jitter-extreme)'}
...
pad_figure(stamp_bad(p_mpg_jitter_extreme), fig.width = 5.5)
```
...
```{r nycflights-points, fig.height = 6 * 0.75, fig.cap = '(ref:nycflights-points)'}
...
pad_figure(stamp_bad(p_flights_scatter), fig.width = 6)
```
...
```{r nycflights-2d-bins, fig.height = 6 * 0.75, fig.cap = '(ref:nycflights-2d-bins)'}
...
pad_figure(p_flights_2d_bins, fig.width = 6)
```
...
```{r nycflights-hex-bins, fig.height = 6 * 0.75, fig.cap = '(ref:nycflights-hex-bins)'}
p_flights_hex_bins <- p_flights_base +
  geom_hex(bins=50) +
  scale_fill_continuous_sequential(palette = "Blue-Yellow", l2 = 90, c2 = 20) +
  theme(legend.position = c(0.85, .85))

pad_figure(p_flights_hex_bins, fig.width = 6)
```
...
```{r midwest-scatter, fig.height = 6 * 0.8, fig.cap = '(ref:midwest-scatter)'}
...
pad_figure(p_midwest_base + geom_point(color = "navy", size = 1, alpha = .7),
           fig.width = 6)
```
...
```{r midwest-density-dots, fig.height = 6 * 0.8, fig.cap = '(ref:midwest-density-dots)'}
p <- p_midwest_base + 
...
```{r midwest-density-smooth, fig.height = 6 * 0.8, fig.cap = '(ref:midwest-density-smooth)'}
p <- p_midwest_base + 
  stat_density_2d(aes(fill = ..level..), geom = "polygon", color = "black", size = 0.2, alpha = 0.5) +
