...
temp_plot <- ggplot(temps_wide, aes(x = date, y = `Houston, TX`)) +
...
tempsplot_F <- ggplot(temps_wide, aes(x = `San Diego, CA`, y = `Houston, TX`)) +
...
tempsplot_C <- ggplot(temps_wide, aes(x = F2C(`San Diego, CA`), y = F2C(`Houston, TX`))) +
  geom_path(size = 1, color = "#0072B2") +
  geom_text_repel(aes(label = label), point.padding = .4, color = "black",
                  min.segment.length = 0) +
