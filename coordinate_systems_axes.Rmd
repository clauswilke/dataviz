...
```{r temperature-normals-Houston, fig.width = 6, fig.asp = 3/4, fig.cap = '(ref:temperature-normals-Houston)'}
...
temp_plot <- ggplot(temps_wide_label, aes(x = date, y = `Houston`)) +
  ...
  scale_y_continuous(limits = c(50, 90),
                     name = "temperature (°F)") +
  theme_dviz_grid(12) +
  theme(plot.margin = margin(3, 12, 12, 1.5))
...
tempsplot_F <- ggplot(temps_wide_label, aes(x = `San Diego`, y = `Houston`)) +
  ...
  theme_dviz_grid() +
  theme(plot.margin = margin(3, 14, 3, 1.5))
...
```{r polar-coord, fig.width = 6, fig.asp = 0.5, fig.cap = '(ref:polar-coord)'}
...
```{r worldmap-four-projections, fig.width = 8.5, fig.cap = '(ref:worldmap-four-projections)'}
library(sf)
