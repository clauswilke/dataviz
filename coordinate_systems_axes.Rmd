...
(ref:cartesian-coord) Standard cartesian coordinate system. (a) The horizontal axis is conventially called *x* and the vertical axis *y*. The two axes form a grid with equidistant spacing. Here, both the *x* and the *y* grid lines are separated by units of one. The point (2, 1) is located two *x* units to the right and one *y* unit above the origin (0, 0). The point (-1, -1) is located one *x* unit to the left and one *y* unit below the origin. (b) The cartesian coordinate system is invariant under rescaling or translation. Here, we have added two to all *x* values (translation) and have multiplied all *y* values by five (rescaling) for the data points in part (a), yet the relative location of the points remains unchanged.

```{r cartesian-coord, fig.width = 8.5, fig.asp = 0.42, fig.cap = '(ref:cartesian-coord)'}
...
p_cart_scaled <- ggplot(df_points_scaled, aes(x, y)) + 
  #geom_path(data = data.frame(x = c(-1, -1, 2, 2), y = c(-1, 0, 0, 1)), linetype = 2) +
  geom_point(size = 3, color = "#0072B2") +
  geom_text(aes(label = label, vjust = vjust, hjust = hjust), size = 12/.pt) +
  #geom_text(data = df_labels, aes(label = label, hjust = hjust, vjust = vjust), size = 12/.pt) +
...
plot_grid(p_cart, p_cart_scaled, labels = "auto")
```
...
temps_wide <- filter(ncdc_normals,
  ...
  arrange(date) %>%
  mutate(label = ifelse(date %in% c(ymd("0000-01-01"), ymd("0000-04-01"), ymd("0000-07-01"), ymd("0000-10-01")),
                        format(date, "%b 1st"), ""))

ggplot(temps_wide, aes(x = `Barrow, AK`, y = `Death Valley, CA`, color = as.numeric(date))) +
  geom_point(size = 1) +
  geom_text_repel(aes(label = label), point.padding = .4, color = "black") +
  coord_fixed(xlim = c(-25, 48), ylim = c(40, 110), expand = FALSE) +
  scale_color_continuous_qualitative(guide = "none") +
  #scale_y_continuous(limits = c(-30, 110), expand = c(0, 0), breaks = seq(-30, 90, by=30), name = "temperature (F)") +
  theme_minimal_grid()
```
