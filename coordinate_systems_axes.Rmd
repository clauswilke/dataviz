...
plotlist <- 
  align_plots(xaxis_lin + scale_x_continuous(limits = c(0, 50)) + 
              ...
              xaxis_sqrt + scale_x_continuous(limits = c(0, 7.07), breaks = sqrt(10*(0:5)), labels = 10*(0:5), name = "x") + 
...
(ref:temperature-normals-cartesian) Daily temperature normals for four selected locations in the U.S. **Caption needs updating.** Source: NOAA.
...
temps_long <- gather(temps_wide, location, temperature, -month, -day, -date) %>%
  filter(location %in% c("Chicago",
                         ...
                         "Houston",
                         "San Diego")) %>%
  mutate(location = factor(location, levels = c("Houston",
                                                "Honolulu",
                                                "San Diego",
                                                "Chicago")))
