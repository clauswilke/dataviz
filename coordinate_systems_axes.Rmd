...
```{r cartesian-coord, fig.height = 6 * 0.8, fig.cap = '(ref:cartesian-coord)'}
...
```{r temperature-normals-Houston, fig.height = 7.5 * 0.618, fig.cap = '(ref:temperature-normals-Houston)'}
...
```{r temperature-normals-Houston-San-Diego, fig.height = 8.5 * 0.5, fig.cap = '(ref:temperature-normals-Houston-San-Diego)'}
...
```{r linear-log-scales, fig.height = 7.5 * 0.618, fig.cap = '(ref:linear-log-scales)'}
...
```{r texas-counties-pop-ratio-log, fig.height = 7.5 * 0.6, fig.cap = '(ref:texas-counties-pop-ratio-log)'}
set.seed(3878)
...
p <- ggplot(tx_counties, aes(x = index, y = popratio)) +
...
```{r texas-counties-pop-ratio-lin, fig.height = 7.5 * 0.6, fig.cap = '(ref:texas-counties-pop-ratio-lin)'}
...
pad_figure(stamp_bad(counties_lin), fig.width = 7.5)
```
...
```{r sqrt-scales, fig.height = 7.5 * 0.464, fig.cap = '(ref:sqrt-scales)'}
...
pad_figure(plot_grid(plotlist[[1]], plotlist[[2]], plotlist[[3]], ncol = 1),
           fig.width = 7.5)
```
...
```{r northeast-state-areas, fig.height = 8.5 * 0.4, fig.cap = '(ref:northeast-state-areas)'}
...
```{r polar-coord, fig.height = 7.5 * 0.5, fig.cap = '(ref:polar-coord)'}
...
pad_figure(plot_grid(p_cart, p_polar, labels = "auto"),
           fig.width = 7.5)
```
...
```{r temperature-normals-polar, fig.height = 7.5 * 0.618, fig.cap = '(ref:temperature-normals-polar)'}
...
p <- ggplot(temps_long, aes(x = date, y = temperature, color = location)) +
  geom_line(size = 1) +
...
```{r continental-usa-four-projections, fig.height = 8.5 * .618, fig.cap = '(ref:continental-usa-four-projections)'}
library(sf)
library(maps)
