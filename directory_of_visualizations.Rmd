...
df_connected_scatter <- data.frame(
  x = c(1.9, 1.5, 2, 3, 3.3, 2.7, 2, 1),
  y = c(0.6, -1, -2.1, -0.9, .6, 1.8, 2, 0.7),
  t = 1:8
)
...
```{r fig.width = 8, fig.asp = 2/4}
palette <- pal_steel_blue

p1 <- ggplot(df_scatter_xy, aes(x, y)) + 
  geom_point(fill = palette[1], color = palette[npal], pch = 21, size = 2.4) + 
...
p4 <- ggplot(df_dense_scatter[sample(1:nrow(df_dense_scatter), 100),]) +
  aes(x, y) +
  geom_smooth(
    color = palette[1],
    fill = palette[npal-2],
...
p5 <- ggplot(filter(df_long, as.integer(var1) < as.integer(var2)),
...
plot_grid(p1, p2, p3, p4, p5, ncol = 4, scale = .9)
```

```{r fig.width = 8, fig.asp = 1/4}
p1 <- ggplot(df_dense_scatter, aes(x, y)) + 
...
p2 <- ggplot(df_dense_scatter, aes(x, y)) + 
...
p3 <- ggplot(df_dense_scatter, aes(x, y)) + 
