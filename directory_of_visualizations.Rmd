...
p1 <- ggplot(df_one_dist, aes(x)) +
  geom_histogram(fill = palette[3], color = palette[npal], bins = 18) +
...
p2 <- ggplot(df_multi_dist, aes(x = type, y = y)) + geom_violin(color = palette[npal], fill = palette[3]) +
...
p5 <- ggplot(df_multi_dist, aes(x = y, fill = factor(type, levels = c("C", "A", "B")))) + 
  geom_histogram(color = palette[npal], bins = 12) +
  scale_fill_manual(values = palette[2:4]) +
...
p7 <- ggplot(df_multi_dist, aes(x = y, y = number, group = number)) + 
  geom_density_ridges(alpha = 0.7, color = palette[npal], fill = palette[2], scale = 2.0) +
