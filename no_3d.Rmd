...
Similar problems arise for other types of 3D plot. Figure \@ref(fig:titanic-3d) is a 3D version of Figure \@ref(fig:titanic-passengers-by-class-sex) from Chapter \@ref(visualizing-amounts). Because of the way the bars are arranged relative to the axes, the bars look much shorter than they actually are. For example, there were 322 passengers total traveling in 1st class, yet Figure \@ref(fig:titanic-3d) suggests that the number was less than 300. 
...
A seemingly more appropriate use of 3D is using three position scales (*x*, *y*, and *z*). In this case, the use of the third dimension is not gratuitous but actually serves a purpose. However, the resulting plots are frequently difficult to interpret. 

As our first example, we will consider a 3D scatter plot. **some more text here.**
Even though we can see this 3D visualization from four different perspectives, it is difficult to envision how exactly the points are distributed in space. I find Figure \@ref(fig:mtcars-3d)d particularly confusing. It almost doesn't seem to show the same data as the other plots, even though nothing has changed other than the angle from which we look at the data.
...
The fundamental problem with 3D visualizations is that they require two separate, successive data transformations. The first transformation maps the data from the data space into the 3D visualization space, as discussed in Chapters \@ref(aesthetic-mapping) and \@ref(coordinate-systems-axes) in the context of position scales. The second maps the data from the 3D visualization space into the 2D space of the final figure, since most data visualizations are ultimately displayed in two dimensions. (This objection obviously does not apply to 3D visualizations in a true 3D environment, such as when shown on a VR display or when produced as physical sculptures or 3D printed objects.) The second transformation is non-invertible, because each point on the 2D display corresponds to a line of points in the 3D visualization space. Therefore, we cannot uniquely determine where in 3D space any particular data point lies.

Our visual system nevertheless attempts to invert the 3D to 2D transformation, as it tries to reconstruct a 3D scene from the 2D image it sees. However, this process is unreliable, fraught with error, and highly dependent on appropriate cues in the image that convey some sense of three-dimensionality. When we remove these cues the inversion becomes entirely impossible. This can be seen in Figure \@ref(fig:mtcars-3d-no-axes), which is identical to Figure \@ref(fig:mtcars-3d) except all depth cues have been removed. The result is four random arrangements of points that we cannot interpret at all and that aren't even easily relatable to each other. Could you tell which points in part (a) correspond to which points in part (b)? I certainly can not.
...
Instead of stacking two separate data transformations, one of which is non-invertible, I think it is generally better to just apply one transformation and map the data directly into 2D space, as I have done throughout the preceding chapters. *talk about lack of need for 3D, have more dimensions than just x and y anyways.*
...
It is virtually always better to replace figures such as Figure \@ref(fig:VA-death-rates-3d) with Trellis plots, where the dataset is sliced along one of the variables shown along the *x* or *y* dimension in the 3D plot. For the Virginia mortality dataset, this type of visualization results in Figure \@ref(fig:VA-death-rates-Trellis). I consider this figure very clear and easy to interpret. It is immediately obvious that mortality rates were higher among men than among women, and also that urban males seem to have had higher mortality rates than rural males when no such trend is apparent for urban and rural females.
...
Finally, it makes sense to use 3D visualizations when we want to show actual 3D objects and/or data mapped onto them. For example, showing the topographic relief of a mountain as a 3D plot is a reasonable choice (Figure \@ref(fig:volcano-relief)). The same is true for a visualization of the evolutionary sequence conservation of a protein mapped onto its 3D structure (Figure \@ref(fig:protein-3d)). In either case, though, these visualizations would also be easier to interpret if they were shown as rotating animations. While this is not possible in traditional print publications, it can be done easily when posting figures on the web or when giving presentations.

(ref:volcano-relief) Relief of Maungawhau (Mount Eden), a volcano in the city of Auckland, New Zealand. Data source: Ross Ihaka

```{r volcano-relief, fig.cap = '(ref:volcano-relief)'}
par(xpd = NA,
    bg = "transparent",
    mai = c(0., 0., 0., 0.),
    family = dviz_font_family_condensed
...
persp3D(z=volcano, col = sequential_hcl(10, palette = "Terrain 2"),
        bty = "b2", shade = 0.15, colkey = FALSE, axes = FALSE,
        ticktype = "detailed", expand = 0.6, phi = 20, theta = 40)
```
