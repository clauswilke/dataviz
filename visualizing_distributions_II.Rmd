...
p <- ggplot(student_data, aes(x = points, y = 50*..y..)) + 
...
p <- ggplot(student_data, aes(x = points, y = 51-50*..y..)) + 
  stat_ecdf(geom = "step", color = "#0072B2") +
...
p <- ggplot(student_data, aes(x = points, y = ..y..)) + 
  stat_ecdf(geom = "step", color = "#0072B2") +
  scale_x_continuous(limits = c(40, 102), expand = c(0, 0), breaks = 10*(4:10)) +
...
```{r county-populations, fig.height = 8.5 * 0.309, fig.cap='(ref:county-populations)'}
...
```{r county-populations-log, fig.height = 8.5 * 0.309, fig.cap='(ref:county-populations-log)'}
...
p <- ggplot(US_census, aes(x=pop2010, y = 1-..y..)) + 
  stat_ecdf(geom = "step", color = "#0072B2", pad = FALSE) +
...
p <- ggplot(student_data, aes(sample = points)) + 
...
p <- ggplot(US_census, aes(sample = log(pop2010))) + 
  geom_abline(slope = 1, intercept = 0, color = "grey70") +
  stat_qq(dparams = params, color = "#0072B2") +
