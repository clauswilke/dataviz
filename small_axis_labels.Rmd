...
stamp_bad(p + theme_minimal_grid(7))
```
...
stamp_ugly(p + theme_minimal_grid(10))
```
...
stamp_phantom(p + theme_minimal_grid())
```
...
stamp_ugly(p + theme_minimal_grid(19) + theme(legend.key.size = grid::unit(1.3, "lines")))
```
...
p2 <- ggplot(male_Aus, aes(x=height, y=pcBfat, color=sport, fill=sport, shape=sport)) +
  ...
  theme(legend.key.size = grid::unit(1.3, "lines"))
