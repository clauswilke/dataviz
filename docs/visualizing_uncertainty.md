...
There are two ways in which we can switch from one hypothetical outcome to the next in a HOP animation: First, we can execute a hard switch, as in a slide projector. We show one outcome for some time period, say 0.5 seconds, and then the next one, and so on. Second, we can smoothly interpolate between outcomes, i.e., slowly deform the trendline for one outcome until it looks like the trendline for another outcome. In my personal opinion, the smooth transition is preferable, because it again reduces the risk of deterministic construal error. By seeing a wide range of intermediate states, even if some of them are not proper hypothetical outcomes, we are reminded that there is a continuum of different outcomes that all could occur.
