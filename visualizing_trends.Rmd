...
As we have seen in the previous section, the behavior of general-purpose smoothers can be somewhat unpredictable for any given dataset. They also do not provide parameter estimates that can be interpreted. Therefore, whenever possible, it is preferable to fit a curve a specific functional form that is appropriate for the data and that contains parameters with clear meaning. 
...
*now linear models: the most commonly used version of curves with defined form.*
...
*now coordinate transformations. whenever possible, transform to linear.*
...
The non-linear fit suggests that preprints grow by a factor of $e^{0.77} = 2.16$ each year, but the linear fit to log-transformed data shows that the annual growth is closer to a factor of $e^{0.88} = 2.41$.
