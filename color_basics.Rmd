...
```{r qualitative-scales, fig.height = 6.5 * 3 * .14, fig.cap = '(ref:qualitative-scales)'}
...
p3 <- gg_color_swatches(7) + scale_fill_hue() + ggtitle("ggplot2 hue")

pad_figure(plot_grid(p1, p2, p3, ncol = 1), fig.width = 6.5)
```
...
```{r popgrowth-US, fig.height = 6.5 * 1.2, fig.cap = '(ref:popgrowth-US)'}
...
p <- ggplot(popgrowth_df, aes(x = state, y = 100*popgrowth, fill = region)) + 
  geom_col() + 
...
```{r sequential-scales, fig.height = 6.5 * 3 * 0.14, fig.cap = '(ref:sequential-scales)'}
...
```{r map-Texas-income, fig.height = 7 * 0.75, fig.cap = '(ref:map-Texas-income)'}
library(sf)

# B19013_001: Median household income in the past 12 months (in 2015 Inflation-adjusted dollars)

p <- ggplot(texas_income, aes(fill = estimate)) + 
  geom_sf(color = "white") + 
  coord_sf(xlim = c(-110, -93.5), datum = NA) +
  theme_map() + 
  scale_fill_distiller(palette = "Blues", type = 'seq', na.value = "grey60", direction = 1,
...
```{r diverging-scales, fig.height = 6.5 * 3 * 0.14, fig.cap = '(ref:diverging-scales)'}
p1 <- gg_color_swatches(7) + scale_fill_discrete_carto(palette = "Earth") + ggtitle("CARTO Earth")
p2 <- gg_color_swatches(7) + scale_fill_brewer(type = "div", palette = "PiYG") + ggtitle("ColorBrewer PiYG")
p3 <- gg_color_swatches(7) + scale_fill_discrete_diverging("Blue-Red") + ggtitle("Blue-Red")
...
```{r map-Texas-race, fig.height = 7 * 0.75, fig.cap = '(ref:map-Texas-race)'}
...
```{r accent-scales, fig.height = 6.5 * 3 * 0.14, fig.cap = '(ref:accent-scales)'}
...
p1 <- gg_color_swatches(7) + scale_fill_manual(values = accent_OkabeIto) + ggtitle("Okabe Ito Accent")
p2 <- gg_color_swatches(7) + scale_fill_manual(values = c("gray60", "gray70","gray80", "gray90", "#C95C4F",   '#83A121', '#6B8AD5')) + ggtitle("Grays with accents")
...
```{r popgrowth-US-highlight, fig.height = 6.5 * 1.2, fig.cap = '(ref:popgrowth-US-highlight)'}
...
p <- ggplot(popgrowth_hilight, aes(x = state, y = 100*popgrowth, fill = region)) + 
  geom_col() + 
  scale_y_continuous(limits = c(-.6, 37.5), expand = c(0, 0),
                     name = "population growth (%)\n2000 to 2010    ") +
...
```{r Aus-athletes-track, fig.height = 7 * 0.618, fig.cap='(ref:Aus-athletes-track)'}
