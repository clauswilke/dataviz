...
stamp_bad(plot_grid(NULL, iris_scatter + theme_minimal_grid(), NULL, nrow=1, rel_widths=c(0.02, 1, 0.04)))
```
...
stamp_phantom(plot_grid(NULL, iris_scatter2 + theme_minimal_grid(), NULL, nrow=1, rel_widths=c(0.02, 1, 0.04)))
```
...
stamp_phantom(plot_grid(NULL, iris_scatter3 + theme_half_open(), NULL, nrow=1, rel_widths=c(0.02, .8, 0.24)))
```
...
xdens <- axis_canvas(iris_scatter2, axis = "x") +
  geom_density_line(data=iris_dens, aes(x = Sepal.Length, y = density, fill = Species),
...
ydens <- axis_canvas(iris_scatter2, axis = "y", coord_flip = TRUE) +
  geom_density_line(data = iris_dens2, aes(x = Sepal.Width, y = density, fill = Species),
                    stat = "identity", alpha = 0.5, size = .2)  +
  scale_fill_manual(values = c("#56B4E9", "#E69F00", "#009E73"),
                    breaks = c("virginica", "versicolor", "setosa"),
                    guide = "none") +
...
p1 <- insert_xaxis_grob(iris_scatter2 + theme_minimal_grid() + 
