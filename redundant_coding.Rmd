...
pad_figure(stamp_bad(iris_scatter))
```
...
```{r iris-scatter-one-shape-cvd, fig.height = 8.5 * 0.65, fig.cap = '(ref:iris-scatter-one-shape-cvd)'}
...
```{r iris-scatter-three-shapes-cvd, fig.height = 8.5 * 0.65, fig.cap = '(ref:iris-scatter-three-shapes-cvd)'}
...
pad_figure(stamp_bad(price_plot_base + theme_minimal_hgrid()))
```
...
pad_figure(price_plot_base_good + theme_minimal_hgrid())
```
...
```{r tech-stocks-good-legend-cvd, fig.height = 8.5 * 0.6, fig.cap = '(ref:tech-stocks-good-legend-cvd)'}
...
```{r iris-scatter-with-ellipses, fig.height = 5.5 * 0.8, fig.cap = '(ref:iris-scatter-with-ellipses)'}
label_df <- data.frame(
...
pad_figure(iris_scatter3, fig.width = 5.5)
```
...
```{r iris-scatter-dens, fig.height = 6.5 * 0.85, fig.cap = '(ref:iris-scatter-dens)'}
# compute densities for sepal lengths
...
```{r temp-ridgeline-colorbar, fig.height = 6 * 0.652, fig.cap = '(ref:temp-ridgeline-colorbar)'}
bandwidth <- 3.4
...
pad_figure(lincoln_final, fig.width = 6)
```
