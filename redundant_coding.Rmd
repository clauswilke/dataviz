...
(ref:iris-scatter-one-shape) Sepal width versus sepal length for three different iris species. A small amount of jitter has been applied to all point positions to prevent points to be plotted on top of each other. 

```{r iris-scatter-one-shape, fig.cap = '(ref:iris-scatter-one-shape)'}
iris_scatter <- ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, fill = Species, color = Species)) + 
...
There are two problems with Figure \@ref(fig:iris-scatter-one-shape): 1. All points have the same shape. 2. The two colors that are more similar to each other (blue and green) are used for the data points that are the most intermingled (virginica and versicolor). We address these two problems by using different shapes and switching out the colors (Figure \@ref(fig:iris-scatter-three-shapes)). Notice how it has become easier to distinguish virginica and versicolor.

(ref:iris-scatter-three-shapes) Sepal width versus sepal length for three different iris species.

```{r iris-scatter-three-shapes, fig.cap = '(ref:iris-scatter-three-shapes)'}
iris_scatter2 <- ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, shape = Species, fill = Species, color = Species)) + 
    geom_point(size = 2.5,
               position = position_jitter(width = 0.01 * diff(range(iris$Sepal.Length)),
                                          height = 0.01 * diff(range(iris$Sepal.Width)),
                                          seed = 3942)) +
...
stamp_phantom(plot_grid(lincoln_final, NULL, rel_widths = c(8, 1)))
```
